...
function getDaysInHebrewMonth(year, monthIndex) {
  // monthIndex: 0: Tishrei, 1: Cheshvan, 2: Kislev, 3: Tevet, 4: Shvat, 5: Adar (or Adar 1), 6: Adar 2, 7: Nisan, 8: Iyyar, 9: Sivan, 10: Tamuz, 11: Av, 12: Elul
  const isLeap = isHebrewLeapYear(year);
  const yearLength = getDaysInHebrewYear(year);

  // Month 0: Tishrei = 30
  if (monthIndex === 0) return 30;
  // Month 1: Cheshvan = 29 or 30 (depends on year length: 355 or 385 has 30)
  if (monthIndex === 1) return (yearLength === 355 || yearLength === 385) ? 30 : 29;
  // Month 2: Kislev = 29 or 30 (353 or 383 has 29)
...
function convertHebrewToGregorian(hebrewDay, hebrewMonthName, hebrewYear) {
  const hYear = hebrewYear ? parseInt(hebrewYear, 10) : 5784;
  const hDay = parseInt(hebrewDay, 10);
  ...
  return {
    day: greg.day,
    month: greg.month,
    year: hebrewYear ? greg.year : undefined
  };
}
